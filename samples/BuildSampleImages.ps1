...
echo "Собираем образ для тестирования"
docker run -e DOCKER_CUSTOM_HOST_IP=$dockerHost ` #Нужно прокинуть хост чтобы из контейнера ходить в зависимости
           -e ASPNETCORE_TEST_CONTENTROOT_SANDBOX=/app/sandbox/ ` #Для TestServer необходимо указать путь к поднимаемому сервису, если он не в тойже папке что и тесты
           -v //var/run/docker.sock:/var/run/docker.sock ` #Мапинг сокетов для докера
           --add-host=docker:$dockerHost `
           -t $test_image_name
echo "Образ с тестами собран"
